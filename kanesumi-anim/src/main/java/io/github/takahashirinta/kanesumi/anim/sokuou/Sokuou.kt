package io.github.takahashirinta.kanesumi.anim.sokuou
...
// tween 预设参数：真实 Metro / UWP 组件的观测时长与曲线聚类。
object SokuouPresets {
    ...
    // 抽屉 / 弹窗入场：慢起快收。300ms + (0.2, 0, 0, 1) —— Metro 面板出场的典型曲线。
    val SheetAppear: AnimationSpec<Float> = tween(
        durationMillis = 300,
        easing = CubicBezierEasing(0.2f, 0f, 0f, 1f)
    )

    // 抽屉 / 弹窗收起：FastOutSlowIn 260ms —— 短促收敛，避免拖尾。
    val SheetDismiss: AnimationSpec<Float> = tween(
        durationMillis = 260,
        easing = FastOutSlowInEasing
    )

    // 快速切换（列表项高亮、播放/暂停 icon）。
