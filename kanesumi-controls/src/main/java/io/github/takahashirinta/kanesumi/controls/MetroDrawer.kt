 ...
 *
 * 动画模型与 [MetroBottomSheet] 同构:单一 `progress: Animatable<Float>` (0=藏,
 * 1=显) 驱动全部视觉,严格零重组:
 * - 入场 0->1 走 SokuouTweens.SheetAppear (300ms MetroCubic);
 ...
 *
 * 宽默认 280dp;系统栏 inset 由内部 metroSystemBarsPadding() 代管,内容侧不用
 * 管。content slot 通常放导航列表;需要横向拖拽关闭的手势暂不内置 -- 没有
 * 真实用例驱动(底部 sheet 的拖拽是平台惯例,侧抽屉不是)。
 */
@Composable
fun MetroDrawer(
    onDismiss: () -> Unit,
